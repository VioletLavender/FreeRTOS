...
void RCC_ClockConfigure(uint32_t SystemClock)
{
    ...
    while(RCC_WaitForFlagStartUp(RCC_FLAG_HSERDY) != SUCCESS);
    ...
    /* PLL = HSE / 1, PLLSRC : HSE */
    RCC_PLLConfig(RCC_HSE_Div1, RCC_PLLMul_2);

    /* Disable PLLON */
    RCC_PLLCmd(DISABLE);

    /* Set PLLMUL and PLLDIV */
    RCC_AutoCalcPLLFactor(SystemClock, 8000000, &PLLDIV, &PLLMUL);

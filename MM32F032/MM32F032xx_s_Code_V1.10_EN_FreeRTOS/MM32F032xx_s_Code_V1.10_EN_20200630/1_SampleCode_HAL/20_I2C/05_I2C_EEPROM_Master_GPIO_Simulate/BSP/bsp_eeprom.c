...
void BSP_EEPROM_Init(void)
{
    ...
    SimulateI2C_Configure(&SimulateI2C, EEPROM_I2C_ADDRESS);
}
...
{
    SimulateI2C_ReadBytes(&SimulateI2C, Addr, pBuf, Len);
}
...
void BSP_EEPROM_WriteBytes(uint8_t Addr, uint8_t *pBuf, uint8_t Len)
{
    ...
    if(StartCnt)
    {
        SimulateI2C_WriteBytes(&SimulateI2C, Addr, pBuf, StartCnt);
    ...
    while(PageNum--)
    {
        SimulateI2C_WriteBytes(&SimulateI2C, Addr, pBuf, EEPROM_PAGE_SIZE);
    ...
    if(FinalCnt)
    {
        SimulateI2C_WriteBytes(&SimulateI2C, Addr, pBuf, FinalCnt);
    }
...
void BSP_EEPROM_Entry(void)
{
    ...
    BSP_EEPROM_WriteBytes(Addr, WriteBuffer, 20);
    ...
    BSP_EEPROM_ReadBytes(Addr, ReadBuffer, 20);

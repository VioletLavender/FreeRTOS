...
void UART1_IRQHandler(void)
{
    if(UART_GetITStatus(UART1, UART_IT_RXIEN) != RESET)
    {
        ...
        if(UART1_RxStruct.CompleteFlag == 0)
        {
            UART1_RxStruct.Buffer[UART1_RxStruct.CurrentCount++] = data;

            if(UART1_RxStruct.CurrentCount == UART1_RxStruct.Length)
            {
                UART1_RxStruct.CompleteFlag = 1;
    ...
    if(UART_GetITStatus(UART1, UART_IT_TXIEN) != RESET)
    {
        ...
        if(UART1_TxStruct.CompleteFlag == 0)
        {
            /* Transmit Data */
            UART_SendData(UART1, UART1_TxStruct.Buffer[UART1_TxStruct.CurrentCount++]);

            if(UART1_TxStruct.CurrentCount == UART1_TxStruct.Length)
            {
                UART1_TxStruct.CompleteFlag = 1;

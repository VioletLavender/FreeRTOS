...
#define FLASH_PROTECTED_PAGES           (FLASH_WRProt_Pages60to63)
 ...
 * @attention   
******************************************************************************/
void FLASH_WriteProtectEntry(void)
{
    uint32_t WRPRValue = 0;

    uint32_t NotProtectedPages;
    uint32_t OldProtectedPages;
    uint32_t NewProtectedPages;
    ...
    /* Get The Flash Write Protection Option Bytes Register Value */
    WRPRValue = FLASH_GetWriteProtectionOptionByte();

    printf("\r\n\r\nWRP : 0x%08x", WRPRValue);

#if 1   /* Disable Write Protect */

    NotProtectedPages =  WRPRValue;
    OldProtectedPages = ~NotProtectedPages;
    NewProtectedPages =  OldProtectedPages & ~FLASH_PROTECTED_PAGES;

    printf("\r\n\r\nCurrent Protected Pages : 0x%08x\r\n", OldProtectedPages);

    if((OldProtectedPages & FLASH_PROTECTED_PAGES) == FLASH_PROTECTED_PAGES)
    {
        /* Erase Option Bytes */
        FLASH_EraseOptionBytes();

        if(NewProtectedPages != 0)
        {
            /* Disable The Selected Pages Write Protected */
            FLASH_EnableWriteProtection(NewProtectedPages);
        }
...
#else   /* Enable Write Protect */

    NotProtectedPages  =  WRPRValue;
    OldProtectedPages  = ~NotProtectedPages;
    NewProtectedPages  =  OldProtectedPages | FLASH_PROTECTED_PAGES;

    printf("\r\n\r\nNew Protected Pages : 0x%08x\r\n", NewProtectedPages);
    ...
    if((OldProtectedPages & FLASH_PROTECTED_PAGES) != FLASH_PROTECTED_PAGES)
    {
        ...
        /* Enable The Selected Pages Write Protected */
        status = FLASH_EnableWriteProtection(NewProtectedPages);
    ...
    if((WRPRValue & FLASH_PROTECTED_PAGES) != 0)
    {
